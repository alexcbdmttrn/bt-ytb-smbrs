...
YOUTUBE_USER_TOKEN = (
    ...
    if os.getenv("YOUTUBE_USER_TOKEN") else {}
)
MAKE_WEBHOOK_URL_REELS = os.getenv("MAKE_WEBHOOK_URL_REELS")
...
def construir_modificadores_epoca(anio):
    if anio is None or anio >= 2015:
        return ("present day contemporary era (2020s): modern vehicles, modern architecture, "
                "modern clothing, LED lighting, smartphones era")
    if anio >= 2000:
        return (f"year {anio} era (early 2000s): 2000s cars, CRT televisions, flip cellphones, "
                "2000s fashion and architecture, no smartphones")
    if anio >= 1990:
        return (f"year {anio} era (1990s): 90s cars, cassette players, CRT TVs, analog phones, "
                "90s fashion, older architecture, no smartphones, no modern tech")
    if anio >= 1980:
        return (f"year {anio} era (1980s): 80s cars, analog rotary phones, period clothing, "
                "older buildings, no modern technology")
    return (f"year {anio} era: old classic cars, analog technology, period clothing, "
...
def actualizar_epoca(anio):
    ...
    try:
        a = int(anio)
        ANIO_SUCESO = a if 1900 <= a <= 2026 else None
    except Exception:
        ANIO_SUCESO = None
    EPOCA_MOD = construir_modificadores_epoca(ANIO_SUCESO)
    print(f"🗓️ Época del suceso: {ANIO_SUCESO if ANIO_SUCESO else 'actualidad (2020s)'}")
...
# ================================================================
PALETAS_COLOR = [
    "Cold cyan blue fog, navy blue shadows, crisp moonlight",
    "Emerald green twilight, dark forest haze, muted sage lighting",
...
# ================================================================
# 🧑 GENERADOR DE PERSONAJES Y UBICACIÓN
# ================================================================
def generar_perfil_personaje_shorts():
    ...
    rasgos = [
        ...
        "with olive skin and a strong jaw", "with pale skin and green eyes", "with tan skin and a warm smile",
    ]
    profesiones = [
        "trailero conduciendo un tráiler en autopista nocturna",
        "policía en su turno nocturno en patrulla", "conductor de taxi en ciudad",
        "repartidor en moto", "velador en condominio residencial", "enfermero en hospital",
        "guardia de seguridad en centro comercial", "carpintero en taller", "paramédico en ambulancia",
    ]
...
def limpiar_prompt_base(prompt, estilo_visual=None, paleta_color=None):
    ...
    if not prompt: prompt = "Night scene, dramatic lighting"
    prompt = re.sub(r"\n+", " ", prompt)
    ...
    modificadores = (
        ...
        "the ENVIRONMENT, objects and location are the main focal point (cars, trees, houses, streets, buildings, gardens, forests), "
...
def cargar_estado():
    try:
        with open(ESTADO_FILE, "r", encoding="utf-8") as f:
            data = json.load(f)
        if "publicaciones_hoy" not in data: data["publicaciones_hoy"] = None
        return data
    except Exception:
...
def guardar_titulo_publicado(titulo):
    ...
    if titulo not in data["titulos"]: data["titulos"].append(titulo)
...
def titulo_ya_publicado(titulo):
    ...
    for t in data["titulos"]:
        ...
        if titulo_norm == t_norm: return True
        palabras1 = set(re.findall(r'\w+', titulo_norm))
        palabras2 = set(re.findall(r'\w+', t_norm))
        if len(palabras1) > 3 and len(palabras2) > 3:
            if len(palabras1.intersection(palabras2)) / min(len(palabras1), len(palabras2)) > 0.7: return True
    return False
...
def incrementar_publicaciones_hoy():
    ...
    hoy = datetime.now(pytz.timezone("America/Mexico_City")).strftime("%Y-%m-%d")
    pub = estado.get("publicaciones_hoy")
    if pub and pub.get("fecha") == hoy: pub["cantidad"] = pub.get("cantidad", 0) + 1
    else: estado["publicaciones_hoy"] = {"fecha": hoy, "cantidad": 1}
...
# ================================================================
# 🖼️ PORTADA VERTICAL SIN TEXTO
# ================================================================
def generar_portada_shorts(intentos=4):
    ubicacion = ESTADO_HISTORIA_SHORTS
    paleta = PALETA_COLOR_ACTUAL
    prompt_final = (
        f"YouTube Shorts horror cover, vertical 9:16: a terrifying eerie scene in {ubicacion}, "
        f"a small lonely figure seen from behind at far distance, a dark ghostly atmosphere, "
        f"wide environmental shot showing the full location, high contrast dramatic lighting, "
        f"cinematic horror style, sharp focus, color palette of {paleta}, "
        f"{EPOCA_MOD}, period-accurate setting. "
        "WIDE shot, environment is the main subject. "
        "EXACTLY ONE person, NO clones, NO duplicates, NO double faces. "
    ...
    negative = (
        ...
        "multiple people, crowd, deformed, mutated, bad anatomy, asymmetrical eyes, "
        "gore, blood, wounds, zombie-like, corpse-like, low quality, blurry"
    )
    url = "https://apihub.agnes-ai.com/v1/images/generations"
    headers = {"Authorization": f"Bearer {AGNES_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": "agnes-image-2.1-flash", "prompt": prompt_final[:1000], "negative_prompt": negative, "width": 1080, "height": 1920, "num_images": 1}
    for intento in range(intentos):
        try:
            print(f"🖼️ Intento {intento+1}/{intentos} portada vertical (sin texto)...")
            r = requests.post(url, headers=headers, json=payload, timeout=90)
            if r.status_code == 200: return r.json()["data"][0]["url"]
        except Exception as e: print(f"⚠️ Error portada: {e}")
        time.sleep(6)
    return None
...
def expandir_texto_corto(texto_corto, ubicacion, personaje):
    prompt = f"""Expande el siguiente relato a 150-170 palabras con detalles sensoriales de {ubicacion}. Mantén trama y tono. Sin CTA. RELATO: {texto_corto}. Devuelve SOLO el relato."""
    ...
    except Exception: return texto_corto
...
def truncar_texto_largo(texto, max_palabras=170):
    ...
    for i in range(max_palabras, max_palabras - 30, -1):
        if i < len(palabras) and palabras[i-1].endswith(('.', '!', '?')): return ' '.join(palabras[:i])
    return ' '.join(palabras[:max_palabras])

# ================================================================
# 🎬 GENERAR HISTORIA (SEO + ÉPOCA DINÁMICA)
# ================================================================
...
🚨 REGLA DE ORO: La historia DEBE estar basada en un relato que REALMENTE alguien contó en internet. Adáptalo en primera persona, tono coloquial, ambientado en {ESTADO_HISTORIA_SHORTS}, México.
🗓️ REGLA CRÍTICA: AÑO DEL SUCESO. Devuelve "anio_suceso": el AÑO (4 dígitos) en que ocurrió el suceso. Si el relato original menciona un año específico, usa ESE año. Si NO se menciona, INVENTA un año coherente con el tipo de suceso (leyendas antiguas → 1970-1995, sucesos urbanos → 2000-2026).
🎯 REGLA CRÍTICA: CONTINUIDAD VISUAL NARRATIVA. El relato se dividirá en 4-5 segmentos visuales con TRAYECTORIA LÓGICA. El ENTORNO es el protagonista: carros, casas, bosques, calles, edificios, jardines según lo que mencione cada segmento.
PROTAGONISTA: {ARTICULO_SHORTS} {PERSONAJE_SHORTS}.
🎯 REGLA CRÍTICA DE LONGITUD: EXACTAMENTE entre 150 y 170 palabras.
📐 ESTRUCTURA: 1. GANCHO (5-10 palabras) 2. CONTEXTO (20-30 palabras) 3. TENSIÓN (80-90 palabras) 4. TWIST FINAL (30-40 palabras)
🎯 REGLA CRÍTICA 1: TÍTULO SEO DE ALTO CTR. FÓRMULA: [VERBO EN 1RA PERSONA / PALABRA DE IMPACTO] + [LUGAR ESPECÍFICO] + [GANCHO EMOCIONAL]. Longitud: 55-75 caracteres.
🎯 REGLA CRÍTICA 2: PALABRAS DE PORTADA (máximo 2 palabras cortas).
🎯 REGLA CRÍTICA 3: DESCRIPCIÓN SEO. Línea 1 (GANCHO máx 90 chars), Línea 2 (CONTEXTO), Línea 3 (CTA canal), Línea 4 (FUENTE), Línea 5 (FACEBOOK), Línea 6 (HASHTAGS máx 5).
🎯 REGLA CRÍTICA 4: TAGS SEO (10-15, máx 480 chars).
🎯 REGLA CRÍTICA 5: TÍTULO ALTERNATIVO (A/B testing).
🚫 TÍTULOS YA PUBLICADOS (NO REPETIR NI PARECERSE): {titulos_referencia}
REGLAS GENERALES: PRIMERA FRASE = GANCHO IMPACTANTE de máximo 5 palabras. Ortografía perfecta. NO repitas frases. "texto_completo" NO debe incluir CTA de suscripción.
...
"tags": "10-15 tags separados por coma (máximo 480 caracteres)"
    ...
    url = "https://api.deepseek.com/v1/chat/completions"
    headers = {"Authorization": f"Bearer {DEEPSEEK_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": "deepseek-chat", "messages": [{"role": "user", "content": prompt}], "temperature": 0.7, "max_tokens": 1100, "response_format": {"type": "json_object"}}
    for intento in range(6):
        try:
            ...
            if "texto_completo" not in data or len(data["texto_completo"]) < 100: raise ValueError("Texto demasiado corto")
            data["texto_completo"] = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]', '', data["texto_completo"])
            actualizar_epoca(data.get("anio_suceso"))
            titulo = data.get("titulo", "").strip()
            titulo = re.sub(r'#\w+', '', titulo).strip()
            titulo = ' '.join(titulo.split())
            if len(titulo) < 40: titulo = f"{titulo} - Testimonio real en {ESTADO_HISTORIA_SHORTS}"
            if len(titulo) > 95: titulo = titulo[:92].rsplit(' ', 1)[0] + "..."
            data["titulo"] = titulo
            if titulo_ya_publicado(titulo): raise ValueError("Título duplicado")
            gancho = data.get("gancho_descripcion", "").strip()
            if not gancho or len(gancho) > 110: gancho = f"Esto fue lo que viví en {ESTADO_HISTORIA_SHORTS} y nunca pude explicar"[:100]
            data["gancho_descripcion"] = gancho
            contexto = data.get("contexto_descripcion", "").strip()
            if not contexto: contexto = f"Un testimonio real de fenómenos paranormales en {ESTADO_HISTORIA_SHORTS}, México."
            data["contexto_descripcion"] = contexto
            fuente = data.get("fuente_relato", "").strip()
            if not fuente: fuente = "Basado en un testimonio real compartido en internet."
            data["fuente_relato"] = fuente
            tags_raw = data.get("tags", "")
            tags_list = [t.strip() for t in tags_raw.split(",") if t.strip()][:15]
            extras_long_tail = [f"terror en {ESTADO_HISTORIA_SHORTS.lower()}", "testimonios paranormales reales", "historias reales contadas en primera persona", "leyendas urbanas mexicanas reales", "casos paranormales reales mexico", "historias de fantasmas reales", "shorts terror"]
            i = 0
            while len(tags_list) < 10 and i < len(extras_long_tail):
                if extras_long_tail[i] not in tags_list: tags_list.append(extras_long_tail[i])
                i += 1
            tags_final = []
            total_chars = 0
            for t in tags_list:
                costo = len(t) + 2
                if total_chars + costo > 480: break
                tags_final.append(t)
                total_chars += costo
            ...
            print(f"   🗓️ Año del suceso: {data.get('anio_suceso', 'N/A')}")
            return data
        except Exception as e:
            print(f"❌ Intento {intento+1}/6 falló: {e}")
            if intento < 5: time.sleep(10 + intento * 5)
    print("❌ TODOS LOS INTENTOS DE GENERACIÓN FALLARON.")
    sys.exit(1)

# ================================================================
# 🆕 DIVIDIR TEXTO Y ASIGNAR ETAPAS
# ================================================================
def dividir_en_segmentos(texto, max_palabras_por_segmento=45):
    oraciones = re.split(r'(?<=[.!?¿¡])\s+', texto)
    oraciones = [o.strip() for o in oraciones if o.strip()]
    if not oraciones: return [texto]
    segmentos, segmento_actual, palabras_actuales = [], [], 0
    for oracion in oraciones:
        ...
        if palabras_actuales + palabras_oracion > max_palabras_por_segmento and segmento_actual:
            ...
            segmento_actual, palabras_actuales = [oracion], palabras_oracion
        else:
            segmento_actual.append(oracion)
            palabras_actuales += palabras_oracion
    if segmento_actual: segmentos.append(" ".join(segmento_actual))
    return segmentos
...
def asignar_etapas_visuales(segmentos, ubicacion):
    ...
    for i in range(n):
        ...
        if progreso < 0.2: etapa, ubic = "inicio_casa", f"interior del hogar en {ubicacion}"
        elif progreso < 0.4: etapa, ubic = "desplazamiento", f"calle o vehículo en movimiento, {ubicacion}"
        elif progreso < 0.65: etapa, ubic = "lugar_destino", f"lugar específico del suceso en {ubicacion}"
        elif progreso < 0.85: etapa, ubic = "climax_evento", f"mismo lugar del suceso en {ubicacion}, momento del evento"
        else: etapa, ubic = "resolucion", f"salida o regreso desde el lugar, {ubicacion}"
        etapas.append(etapa)
...
def generar_prompt_con_contexto(segmento_texto, etapa, ubicacion_escena, segmento_anterior_texto=None, perfil=None, estilo_visual=None, paleta_color=None):
    ...
    contexto_previo = f"\nPREVIOUS SCENE: The character was just in: '{segmento_anterior_texto[:120]}'" if segmento_anterior_texto else ""
    instrucciones_etapa = {
        "inicio_casa": "Show the home interior or the character at home, establishing shot, calm atmosphere.",
        "desplazamiento": "Show the character in movement (walking, driving, in a vehicle), same route continuing from previous scene.",
        "lugar_destino": "Show the character arriving at or exploring the specific location, maintaining architectural consistency.",
        "climax_evento": "Show the paranormal event happening in this exact location, environment as main subject.",
...
Fragmento del relato actual: \"\"\"{segmento_texto}\"\"\"
{contexto_previo}
...
- El ENTORNO es el PROTAGONISTA: carros, casas, calles, bosques, jardines, edificios según lo que mencione el segmento.
- Personaje: {perfil}, ocupando como MÁXIMO el 20% del área, a distancia.
- Si el segmento menciona OBJETOS o ENTORNOS, esos son el sujeto principal.
- Estilo: professional hyperrealistic photography, 4k, ultra-detailed.
- Paleta de color: {paleta}
- ÉPOCA: {EPOCA_MOD} - Los vehículos, ropa y tecnología deben corresponder a esa época.
- PROHIBIDO: abandoned, decaying, rusty, crumbling, cracked, dilapidated, vintage, sepia.
VISUAL CONSISTENCY RULES:
- EXACTLY ONE PERSON (the main character), NO clones, NO duplicates, NO double faces
...
Devuelve SOLO el prompt en inglés, sin explicaciones."""
    url = "https://api.deepseek.com/v1/chat/completions"
    headers = {"Authorization": f"Bearer {DEEPSEEK_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": "deepseek-chat", "messages": [{"role": "user", "content": prompt}], "temperature": 0.6, "max_tokens": 300}
    try:
        r = requests.post(url, headers=headers, json=payload, timeout=60)
        r.raise_for_status()
        prompt_imagen = r.json()["choices"][0]["message"]["content"].strip()
        prompt_imagen += f", {estilo}, vertical 9:16, wide establishing shot, person occupies max 20% of frame, environment as main subject, no close-up face, no portrait, no blood, no gore, {EPOCA_MOD}, single person only, no cut off body, no clones, no duplicates"
        return prompt_imagen
    except Exception as e:
        print(f"⚠️ Error generando prompt de imagen: {e}")
        return f"Wide establishing shot of {ubicacion_escena}, depicting: {segmento_texto[:100]}, {estilo}, vertical 9:16, no close-up face, environment as main subject, {EPOCA_MOD}, single person, no cut off body"
...
def generar_imagen_vertical(prompt, intentos=3):
    ...
    negative = (
        ...
        "rusty, rusted, oxidized, weathered, aged, antique, old-fashioned, "
    ...
    payload = {"model": "agnes-image-2.1-flash", "prompt": prompt_limpio, "negative_prompt": negative, "width": 1080, "height": 1920, "num_images": 1}
...
def generar_recursos_por_segmento(segmentos, etapas, ubicaciones, perfil, ubicacion, estilo, paleta, intentos_por_imagen=3):
    ...
    for idx, seg in enumerate(segmentos):
        ...
        prompt_imagen = generar_prompt_con_contexto(seg, etapa, ubic_escena, seg_anterior, perfil, estilo, paleta)
        img_url = None
        for intento in range(intentos_por_imagen):
            ...
            if intento < intentos_por_imagen - 1: time.sleep(6)
        if not img_url: print(f"    ⚠️ Imagen falló, se usará la siguiente imagen disponible")
        ...
        try:
            ...
            audio_clip.close()
        except Exception: duracion = 10.0
        resultados_temporales.append({"imagen_url": img_url, "audio_path": audio_path, "duracion": duracion})
        if idx < len(segmentos) - 1: time.sleep(12)
        ...
        if res["imagen_url"] is None:
            ...
            for j in range(i + 1, len(resultados_temporales)):
                if resultados_temporales[j]["imagen_url"] is not None:
                    ...
                    break
            if siguiente_imagen is not None: res["imagen_url"] = siguiente_imagen
            elif i > 0 and resultados_temporales[i-1]["imagen_url"] is not None: res["imagen_url"] = resultados_temporales[i-1]["imagen_url"]
            else:
                img_url = generar_placeholder_local("Terror", (1080, 1920))
                if not img_url: img_url = "https://via.placeholder.com/1080x1920/1a1a1a/ff0000?text=Terror"
...
def generar_audio(texto, index, intentos_por_voz=2):
    ...
    if len(texto_limpio) < 30: texto_limpio = "Esa noche en la carretera, el silencio era tan denso que podía cortarse con un cuchillo. El miedo lo envolvía todo. No podía escapar."
    ...
    for intento_voz, voz_config in enumerate(voces_a_probar):
        voz, rate, pitch = voz_config["voz"], voz_config["velocidad"], voz_config["tono"]
        for intento in range(intentos_por_voz):
            ...
            try:
                ...
                if os.path.exists(filename) and os.path.getsize(filename) > 0:
                    ...
                    return filename
            except Exception as e: print(f"❌ Falló con {voz}: {e}")
            if intento < intentos_por_voz - 1: time.sleep(3 * (intento + 1))
...
def montar_video_shorts(recursos_por_segmento, fondo_path, salida="short_final.mp4"):
    if not recursos_por_segmento: raise ValueError("No hay recursos para montar el video")
    clips_video, clips_audio = [], []
    for i, recurso in enumerate(recursos_por_segmento):
        img_url, audio_path, duracion = recurso["imagen_url"], recurso["audio_path"], recurso["duracion"]
        try:
            audio_clip = AudioFileClip(audio_path)
            clips_audio.append(audio_clip)
        except Exception as e: raise ValueError(f"Fallo al cargar audio del segmento {i}")
        try:
            ...
            with Image.open(img_path) as img:
                ImageOps.fit(img, (1080, 1920), Image.Resampling.LANCZOS).save(img_path)
            clips_video.append(ImageClip(img_path).set_duration(duracion))
        except Exception:
            placeholder = generar_placeholder_local(f"Img {i+1}")
            if placeholder:
                with Image.open(placeholder) as img:
                    ImageOps.fit(img, (1080, 1920), Image.Resampling.LANCZOS).save(placeholder)
                clips_video.append(ImageClip(placeholder).set_duration(duracion))
            else: continue
    if not clips_video: raise ValueError("No se pudieron crear clips de video")
    PAUSA_ENTRE_SEGMENTOS = 0.3
    if len(clips_audio) > 1:
        audio_con_pausas = []
        for i, audio in enumerate(clips_audio):
            audio_con_pausas.append(audio)
            if i < len(clips_audio) - 1: audio_con_pausas.append(AudioClip(lambda t: 0, duration=PAUSA_ENTRE_SEGMENTOS))
        audio_narracion = concatenate_audioclips(audio_con_pausas)
    else: audio_narracion = clips_audio[0]
    ...
    if fondo_path and os.path.exists(fondo_path):
        ...
        except Exception: audio_final = audio_narracion
    else: audio_final = audio_narracion
    ...
    if 'fondo_clip' in locals(): fondo_clip.close()
    if cta_audio_path and os.path.exists(cta_audio_path):
        try: os.remove(cta_audio_path)
        except: pass
    return salida
...
{hashtags_descripcion}"""
    ...
    body = {
        "snippet": {
            "title": titulo, "description": descripcion[:5000], "tags": etiquetas[:30],
    ...
    for _ in range(intentos):
        try:
            with open(video_path, "rb") as f:
                r = requests.post("https://litterbox.catbox.moe/resources/internals/api.php", data={"reqtype": "fileupload", "time": "72h"}, files={"fileToUpload": f}, timeout=180)
...
def enviar_a_make(titulo, descripcion, video_url, url_youtube=""):
    ...
    try:
        r = requests.post(MAKE_WEBHOOK_URL_REELS, json=payload, timeout=60)
...
# ================================================================
def main():
    print("🎬 Iniciando Bot de SHORTS (Relatos REALES + Época dinámica)")
    ...
    if publicadas_hoy >= META_DIARIA_SHORTS:
        print(f"✅ Meta de {META_DIARIA_SHORTS} shorts alcanzada. Saliendo.")
        sys.exit(0)
    estado = cargar_estado()
    fondo_path = seleccionar_fondo_disponible(estado)
    historia_raw = generar_historia_completa()
    if not historia_raw: sys.exit(1)
    texto_completo = historia_raw.get("texto_completo", "")
    palabras = len(texto_completo.split())
    if palabras < 130: texto_completo = expandir_texto_corto(texto_completo, ESTADO_HISTORIA_SHORTS, PERSONAJE_SHORTS)
    elif palabras > 190: texto_completo = truncar_texto_largo(texto_completo, max_palabras=170)
    perfil = PERFIL_PERSONAJE_SHORTS
    ubicacion = ESTADO_HISTORIA_SHORTS
    paleta = PALETA_COLOR_ACTUAL
    estilo = ESTILO_VISUAL_ACTUAL
    print(f"\n📊 RESUMEN SEO:")
    print(f"   🏷️ Título: {historia_raw['titulo']}")
    print(f"   🗓️ Año del suceso: {historia_raw.get('anio_suceso', 'N/A')}")
    ...
    recursos = generar_recursos_por_segmento(segmentos, etapas, ubicaciones, perfil, ubicacion, estilo, paleta, intentos_por_imagen=3)
    if not recursos:
        print("❌ Error generando recursos. Abortando.")
        sys.exit(1)
    print("\n🖼️ Generando portada vertical SIN texto...")
    portada_url = generar_portada_shorts()
    if portada_url and recursos:
        try:
            r = requests.get(portada_url, timeout=30)
            r.raise_for_status()
            portada_path = "portada_short.jpg"
            with open(portada_path, "wb") as f: f.write(r.content)
            ...
            print("✅ Portada SIN texto aplicada como PRIMER frame del Short.")
        except Exception as e: print(f"⚠️ Error aplicando portada: {e}")
    else: print("⚠️ No se generó portada; se usa la imagen del segmento 1.")
    try: video_final = montar_video_shorts(recursos, fondo_path, salida="short_final.mp4")
    except Exception as e:
        print(f"❌ Error montando video: {e}")
        sys.exit(1)
    video_id_youtube = subir_a_youtube(
        video_path=video_final, titulo=historia_raw["titulo"], etiquetas=historia_raw["tags"],
        gancho_descripcion=historia_raw["gancho_descripcion"], contexto_descripcion=historia_raw["contexto_descripcion"],
...
{historia_raw['hashtags_descripcion']}"""
            enviar_a_make(titulo=historia_raw["titulo"], descripcion=descripcion_facebook, video_url=video_url_temporal, url_youtube=f"https://youtu.be/{video_id_youtube}")
        else: print("⚠️ No se pudo subir al host temporal. Facebook omitido.")
    ...
    print("✨ Ejecución del Bot finalizada con época dinámica y portada sin texto.")
