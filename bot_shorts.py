...
# ================================================================
# 🗓️ ÉPOCA DINÁMICA DEL SUCESO (variable según relato y segmento)
# ================================================================
ANIO_SUCESO = None
EPOCA_MOD = ""
...
def generar_perfil_personaje_shorts():
    ...
    profesiones = [
        ...
        "conductor de taxi en ciudad", "repartidor en moto",
        "velador en condominio residencial", "enfermero en hospital",
        "guardia de seguridad en centro comercial", "carpintero en taller",
...
# ================================================================
# 🧼 LIMPIADOR DE PROMPTS (entorno protagonista + época + anti-clones)
# ================================================================
def limpiar_prompt_base(prompt, estilo_visual=None, paleta_color=None):
    ...
    modificadores = (
        ...
        "the ENVIRONMENT, objects and location are the main focal point (cars, trees, houses, streets, gardens, buildings), "
...
# ================================================================
# 🖼️ PORTADA VERTICAL SIN TEXTO (solo imagen limpia, con época)
# ================================================================
def generar_portada_shorts(intentos=4):
    ubicacion = ESTADO_HISTORIA_SHORTS
    paleta = PALETA_COLOR_ACTUAL
    prompt_final = (
        f"YouTube Shorts horror cover, vertical 9:16: a terrified Mexican person seen from behind at distance "
        f"looking toward a dark eerie location in {ubicacion}, a faint ghostly silhouette far in the background, "
        f"high contrast dramatic lighting, cinematic horror style, sharp focus, color palette of {paleta}, "
        f"{EPOCA_MOD}, period-accurate setting. "
        "WIDE shot, environment is the main subject, person small at most 20% of frame. "
...
# ================================================================
# 🎬 GENERAR HISTORIA (con anio_suceso + SEO)
# ================================================================
...
- Devuelve "anio_suceso": el AÑO (4 dígitos) en que ocurrió el suceso.
- Si el relato original menciona un año, usa ESE año exacto.
- Si NO se menciona, INVENTA un año coherente con el entorno y tono del relato (leyenda antigua → 1970-1999; suceso moderno → 2015-2026).
🎯 REGLA CRÍTICA: CONTINUIDAD VISUAL NARRATIVA
El relato se dividirá en 4-5 segmentos visuales con etapas: inicio_casa, desplazamiento, lugar_destino, climax_evento, resolucion.
Trayectoria lógica y entorno coherente entre segmentos.
...
FÓRMULA: [VERBO 1RA PERSONA / IMPACTO] + [LUGAR] + [GANCHO EMOCIONAL]
✅ EJEMPLOS: "Vi algo en la carretera de Zacatecas que no era humano", "El Uber me dejó en un lugar que no existe en el mapa"
❌ PROHIBIDOS: "La leyenda de...", "El fantasma de...", "El misterio de..."
Longitud: 55-75 caracteres.
🎯 REGLA CRÍTICA 2: DESCRIPCIÓN SEO
...
🎯 REGLA CRÍTICA 5: TÍTULO ALTERNATIVO.
🚫 TÍTULOS YA PUBLICADOS (NO REPETIR):
{titulos_referencia}
REGLAS GENERALES: PRIMERA FRASE = gancho máx 5 palabras. Ortografía perfecta. NO repitas frases. "texto_completo" SIN CTA.
...
"palabras_portada": "2 palabras gancho",
    ...
    for intento in range(6):
        try:
            ...
            extras = [
                ...
                "testimonios paranormales reales", "historias reales contadas en primera persona",
                "leyendas urbanas mexicanas reales", "relatos de internet reales",
                "casos paranormales reales mexico", "historias de fantasmas reales", "shorts terror",
            ]
            i = 0
            ...
            print(f"   🏷️ Título SEO: {data['titulo']}")
            print(f"   🗓️ Año suceso: {data.get('anio_suceso')}")
            return data
        except Exception as e:
...
# ================================================================
# 🎨 PROMPT DE IMAGEN POR SEGMENTO (entorno protagonista + época + continuidad)
# ================================================================
def generar_prompt_con_contexto(segmento_texto, etapa, ubicacion_escena, segmento_anterior_texto=None, perfil=None, estilo_visual=None, paleta_color=None):
    ...
    instrucciones_etapa = {
        "inicio_casa": "Show the home interior as main subject, establishing shot, calm before the events.",
        "desplazamiento": "Show the vehicle or street as main subject, character in movement small at distance, same route as previous scene.",
        "lugar_destino": "Show the specific location (house, forest, mine, garden, street) as main subject, maintaining consistency with previous scene.",
        "climax_evento": "Show the paranormal event happening in this exact location, environment as protagonist, character reacting small at distance.",
        "resolucion": "Show the aftermath or the location after the event, calmer atmosphere.",
    }
...
\"\"\"
{contexto_previo}
Genera un PROMPT DE IMAGEN EN INGLÉS para una foto vertical (9:16) que ilustre EXACTAMENTE lo que pasa en este fragmento.
REGLA DE PROTAGONISTA:
- Identifica qué menciona el fragmento: OBJETOS/ENTORNOS (carro, casa, bosque, jardín, calle, mina, río, etc.) o ACCIONES de una persona.
- Si el fragmento se centra en objetos o entornos: ELLOS son el sujeto principal de la imagen. NO incluyas personas (o una persona diminuta muy al fondo, opcional).
- Si el fragmento menciona a una persona haciendo algo: incluye EXACTAMENTE UNA persona: {perfil}, ocupando MÁXIMO 20% del encuadre, a distancia, de espaldas o de lado.
- La imagen debe CONTINUAR visualmente la escena anterior (misma ubicación lógica).
- ÉPOCA: {EPOCA_MOD}. Si el fragmento menciona un salto de tiempo (ej: "hoy, 30 años después"), ajusta la época a ese momento; si no, usa la época indicada.
SCENE CONTINUITY INSTRUCTIONS:
- Current narrative stage: {etapa}
- Current location: {ubicacion_escena}
- DIRECTIVE: {instruccion}
Reglas:
- PLANO: Wide o extreme wide shot. PROHIBIDO close-up, portrait, headshot.
- Estilo: {estilo}
- Paleta: {paleta}
- PROHIBIDO: gore, blood, clones, duplicate people, double faces, two heads, mirror faces.
...
"""
    ...
    try:
        ...
        prompt_imagen += (
            f", {estilo}, vertical 9:16, environment and objects as main subject, "
            "person at most 20% of frame ONLY if the fragment mentions a person, "
            f"{EPOCA_MOD}, exactly one person max, no clones, no duplicates, no double faces, "
            "no close-up face, no portrait, no blood, no gore, no text, no watermark"
        )
        return prompt_imagen
    except Exception as e:
        print(f"⚠️ Error generando prompt de imagen: {e}")
        return f"Wide establishing shot of {ubicacion_escena}, {EPOCA_MOD}, {estilo}, vertical 9:16, environment as main subject, no close-up face, single person max, no text"

# ================================================================
# 🖼️ GENERAR IMAGEN VERTICAL (anti-clones)
# ================================================================
...
def generar_recursos_por_segmento(segmentos, etapas, ubicaciones, perfil, ubicacion, estilo, paleta, intentos_por_imagen=3):
    ...
    for idx, seg in enumerate(segmentos):
        ...
        print(f"  🎬 Segmento {idx+1}/{len(segmentos)} - Etapa: {etapa}")
    ...
    for i, res in enumerate(resultados_temporales):
        if res["imagen_url"] is None:
            ...
            elif i > 0 and resultados_temporales[i - 1]["imagen_url"] is not None:
...
# ================================================================
# ✅ GENERAR AUDIO (una sola voz con fallback)
# ================================================================
...
def montar_video_shorts(recursos_por_segmento, fondo_path, salida="short_final.mp4"):
    ...
    if cta_audio_path and os.path.exists(cta_audio_path):
        try:
            ...
            clips_video[-1] = clips_video[-1].set_duration(clips_video[-1].duration + duracion_cta)
            video = concatenate_videoclips(clips_video, method="compose")
            print(f"✅ CTA final agregado ({duracion_cta:.1f}s)")
        except Exception as e:
    ...
    video.close(); audio_final.close(); audio_narracion.close()
...
# ================================================================
# 🧼 LIMPIADOR DE PROMPTS (ÉPOCA + AMBIENTE PROTAGONISTA + ANTI-CLONES)
# ================================================================
def limpiar_prompt_base(prompt, estilo_visual=None, paleta_color=None):
estilo = estilo_visual or ESTILO_VISUAL_ACTUAL
paleta = paleta_color or PALETA_COLOR_ACTUAL
...
# ================================================================
def generar_portada_shorts(intentos=4):
"""Portada vertical 9:16 SIN letras, limpia, con época del suceso."""
ubicacion = ESTADO_HISTORIA_SHORTS
paleta = PALETA_COLOR_ACTUAL
prompt_final = (
f"YouTube Shorts horror cover, vertical 9:16: a terrifying eerie scene in {ubicacion}, "
"a small lonely figure seen from behind at far distance, a dark ghostly atmosphere, "
"wide environmental shot showing the full location, high contrast dramatic lighting, "
...
# ================================================================
# 🎬 GENERAR HISTORIA (SEO + ÉPOCA DINÁMICA + CONTINUIDAD)
# ================================================================
def generar_historia_completa():
titulos_pub = cargar_titulos_publicados()["titulos"][-10:]
titulos_referencia = "\n".join([f"- {t}" for t in titulos_pub]) if titulos_pub else "Ninguno aún."
...
La historia DEBE estar basada en un relato que REALMENTE alguien contó en internet.
Adáptalo en primera persona, tono coloquial, ambientado en {ESTADO_HISTORIA_SHORTS}, México.
🗓️ REGLA CRÍTICA: AÑO DEL SUCESO
- Devuelve "anio_suceso": el AÑO (4 dígitos) en que ocurrió el suceso.
- Si el relato original menciona un año específico, usa ESE año.
- Si NO se menciona, INVENTA un año coherente con el tipo de suceso (leyendas antiguas → 1970-1995, sucesos urbanos → 2000-2026).
- Las imágenes deben reflejar esa época (autos, ropa, tecnología del año indicado).
🎯 REGLA CRÍTICA: CONTINUIDAD VISUAL NARRATIVA
El relato se dividirá en 4-5 segmentos visuales con TRAYECTORIA LÓGICA.
Cada imagen debe continuar la escena anterior sin perder la coherencia del relato.
El ENTORNO es el protagonista: carros, casas, bosques, calles, edificios, jardines según lo que mencione cada segmento.
PROTAGONISTA: {ARTICULO_SHORTS} {PERSONAJE_SHORTS}.
🎯 REGLA CRÍTICA DE LONGITUD:
- EXACTAMENTE entre 150 y 170 palabras.
...
❌ PROHIBIDOS: "La leyenda de...", "El fantasma de...", "El misterio de..."
Longitud: 55-75 caracteres, primera persona, lugar específico.
...
raise ValueError("Texto demasiado corto o campo faltante")
data["texto_completo"] = re.sub(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]', '', data["texto_completo"])
# 🗓️ ACTUALIZAR ÉPOCA según año del suceso
...
- Si el segmento menciona OBJETOS o ENTORNOS (carros, árboles, casas, bosques, jardines, calles), esos son el sujeto principal.
...
# ================================================================
# 🖼️ GENERAR IMAGEN VERTICAL (ANTI-CLONES AGRESIVO)
# ================================================================
def generar_imagen_vertical(prompt, intentos=3):
prompt_limpio = limpiar_prompt_base(prompt, ESTILO_VISUAL_ACTUAL, PALETA_COLOR_ACTUAL)
url = "https://apihub.agnes-ai.com/v1/images/generations"
...
# ================================================================
# ✅ GENERAR AUDIO (FALLBACK NEURAL, SIN gTTS)
# ================================================================
def generar_audio(texto, index, intentos_por_voz=2):
global CONFIG_VOZ_ACTUAL
texto_limpio = re.sub(r"imagen_prompt.*", "", texto, flags=re.IGNORECASE).strip()
...
try:
video_final = montar_video_shorts(recursos, fondo_path, salida="short_final.mp4")
except Exception as e:
